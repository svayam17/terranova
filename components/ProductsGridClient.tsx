        ...
        <div className="grid gap-8 sm:grid-cols-2 xl:grid-cols-5">
          {collections.map((collection) => (
            <div key={collection.slug} className="group">
              <div className="relative h-[320px] sm:h-[380px] xl:h-[460px] overflow-hidden">
              ...
              <div className="mt-4 flex items-center justify-between">
                <span className="font-serif text-lg text-charcoal">{collection.name}</span>
                <Link href={`/products/${collection.slug}`} className="text-charcoal/60 transition-transform duration-300 group-hover:translate-x-1 group-hover:-translate-y-1 inline-flex items-center">
                  <ArrowUpRight />
                </Link>
              </div>
            </div>
